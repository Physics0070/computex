...
**[TRANSACTIONS.md](TRANSACTIONS.md)** is the record of every USDC payment this project has settled, with a link to each transaction on the public explorer.

It is generated by `npm run tx:log`, which reads the payer and payee accounts from the Algorand indexer — not from the server, whose job store is in-memory and resets on restart. The chain is the only durable and independently verifiable ledger here, so the file is regenerated from it rather than accumulated locally. Re-run it after a demo to pick up new payments.
...
│  └─ scripts/              keygen, optin, check-payer, test-flow, tx-history, dev helpers
└─ client/
   └─ src/
